...
// Public dRPC endpoint - stable and compatible
        ...
        const req = https.request({
            ...
            res.on('end', () => {
                try {
                    ...
                    if (parsed.error) console.log(`  [RPC Response Error]: ${JSON.stringify(parsed.error)}`);
                    resolve(parsed);
                } catch (e) {
                    console.error('  [RPC Fatal] Failed to parse JSON:', data.slice(0, 200));
                    reject(new Error('Invalid JSON response'));
                }
            });
        });
        req.on('error', (e) => {
            console.error('  [RPC Network Error]:', e.message);
...
async function waitForTx(txHash) {
    ...
    for (let i = 0; i < 80; i++) {
        ...
        // Using named param for receipt on dRPC
        ...
        if (res.result && res.result.finality_status && res.result.finality_status !== 'RECEIVED') {
            console.log('\n  ✅ TX Confirmed: ' + res.result.finality_status);
            return res.result;
        }
...
async function main() {
    console.log('--- RAW STARKNET DEPLOYMENT (dRPC Named Params) ---\n');
    const targetDir = path.join(process.cwd(), 'contracts_v1/target/dev');

    // 0. Nonce (named params)
    ...
    for (const name of contracts) {
        ...
        if (!fs.existsSync(sierraPath)) throw new Error('Missing file: ' + sierraPath);
        ...
        if (classCheck.result) {
            ...
            console.error('  [Fatal getClass Response]:', JSON.stringify(classCheck));
            throw new Error('getClass unexpected error');
        }
