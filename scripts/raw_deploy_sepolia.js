...
const { hash, json, num, encode, ec, CallData, stark, constants } = require('starknet');
        ...
        const req = https.request({
            ...
            res.on('end', () => {
                try {
                    ...
                    if (parsed.error) {
                        console.log(`  [RPC Error Response]: ${JSON.stringify(parsed.error)}`);
                    }
                    resolve(parsed);
                } catch (e) {
...
async function main() {
    console.log('--- RAW STARKNET V2/V1 DEPLOYMENT (DRPC) ---\n');
    ...
    for (const name of contracts) {
        ...
        if (classCheck.result) {
            ...
            throw new Error('getClass error: ' + JSON.stringify(classCheck.error));
        }
