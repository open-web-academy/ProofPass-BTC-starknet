...
async function deployContract(account, name, constructorCalldata = []) {
    ...
    const fee = '0x2386f26fc10000'; // 0.01 ETH

    try {
        // v6 declare with version 2 and hardcoded maxFee
        console.log(`  Declaring...`);
        const d = await account.declare({ contract: sierra, casm }, { version: 2, maxFee: fee });
