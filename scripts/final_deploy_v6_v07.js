...
async function deployContract(account, name, constructorCalldata = []) {
    ...
    // Explicit maxFee to avoid library-led estimation using V3
    const maxFee = '0x2386f26fc10000'; // 0.01 ETH

    try {
        console.log(`  Declaring (V2)...`);
        const d = await account.declare({ contract: sierra, casm }, { version: 2, maxFee });
...
async function main() {
    // Force Spec 0.7 to avoid V3 schema mismatches
    const provider = new RpcProvider({ nodeUrl: RPC, chainId: constants.StarknetChainId.SN_SEPOLIA });
    // In some v6 versions, we can set the spec version
    if (provider.channel && provider.channel.setSpecVersion) {
        provider.channel.setSpecVersion('0.7');
    }
