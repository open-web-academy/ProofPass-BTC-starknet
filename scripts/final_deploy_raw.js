...
// Alchemy v2 URL (v0.7 RPC compatible)
...
async function waitForTx(txHash) {
    ...
    for (let i = 0; i < 80; i++) {
        ...
        const res = await rpc('starknet_getTransactionReceipt', [txHash]);
...
async function main() {
    console.log('--- RAW STARKNET DEPLOYMENT (v0.7 RPC) ---\n');
    const targetDir = path.join(process.cwd(), 'contracts_v1/target/dev');

    // 0. Nonce
    const nonceResp = await rpc('starknet_getNonce', ['latest', ACCOUNT_ADDR]);
    if (nonceResp.error) throw new Error('Nonce fetch failed');
    ...
    for (const name of contracts) {
        ...
        // 1. Declare V2
        console.log('  Checking if declared...');
        const classCheck = await rpc('starknet_getClass', ['latest', classHash]);
        ...
        if (classCheck.result) {
            ...
            const res = await rpc('starknet_addDeclareTransaction', [declareTx]);
        ...
        const resInvoke = await rpc('starknet_addInvokeTransaction', [invokeTx]);
        ...
        // Find UDC event manually if needed, or just assume success if confirmed
